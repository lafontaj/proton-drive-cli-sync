...
- **Mode « local seul » (sans NAS) + configurabilité des chemins.** Le cœur supporte déjà le local (le consommateur tolère une file NAS absente, `mount_check` gère les sources `local`, le watcher local surveille les sources locales, les suppressions locales ne sont pas soumises au garde-fou NFS — les mappings locaux actuels le prouvent). À faire : (a) config centralisée généralisant le modèle `PROTON_DRIVE_CLI` (emplacement d'installation, chemin du CLI, présence NAS oui/non, point de montage NAS, préfixes NAS, chemin de la file NAS) ; (b) NAS optionnel (lecture de la file NAS et déploiement de `nas_watcher.py` conditionnels) ; (c) GUI adaptatif masquant les sections NAS sans NAS ; (d) README à deux modes ; décision à trancher : auto-détection unique vs deux configurations. Chemins codés en dur recensés : `nas_watcher.py` (`BASE_DIR`, `NAS_PREFIXES`), `realtime_consumer.py` (file NAS, `ENGINE`), `realtime_manager.py` (`APP_DIR`, `NAS_MOUNT`, `DEFAULT_CLI`), `proton_mapping_editor.py`/`schedule_manager.py` (`DEFAULT_CLI`/`ENGINE`/`LOG_DIR`), `proton_sync.py` (`CLI`). Synergie avec l'anonymisation GitHub (ces chemins contiennent l'IP du NAS, le hostname, les noms réels).
- **Lancer la synchro sur une sélection de mappings.** La multi-sélection ajoutée pour l'amorçage crée une ambiguïté : « Lancer la synchro » traite tout le fichier, mais l'utilisateur pourrait croire qu'une sélection unique est possible. À faire : permettre de lancer la synchro sur les mappings sélectionnés (réutiliser `--only-source`), pour uniformiser le comportement des deux boutons.
- **Régénérer les PDF** (`build_pdf.py`) une fois les nouveaux comportements documentés ; revoir le mapping de substitution des symboles maintenant que les glyphes s'affichent correctement.
- **Finalisation GitHub** : anonymisation (courriel, noms réels, hostname, IP NAS, identifiants, chemins), licence MIT, `.gitignore` (exclure le binaire CLI), avis de non-affiliation, README bilingue.
