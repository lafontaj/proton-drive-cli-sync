...
- **Version de CLI testée : 0.6.0.** Le comportement du CLI change d'une version à l'autre, et seule celle-ci a été validée ici — les capacités décrites plus bas (type de média correct même avec une extension en majuscules, mise à la corbeille dans un dossier « Partagé avec moi ») sont apparues en 0.5.0 et restent acquises. L'application vérifie la version installée au démarrage et avertit si elle diffère (antérieure *ou* postérieure), en proposant de quitter pour installer la version testée. Le moteur, qui tourne aussi sans surveillance depuis les timers, se contente d'une ligne d'avertissement et n'interrompt jamais un passage.
