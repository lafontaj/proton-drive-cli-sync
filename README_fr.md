...
**1. Chemins de données identiques des deux côtés — ou une correspondance déclarée.** Le plus simple est que les dossiers surveillés soient accessibles au **même chemin** sur la machine locale et sur le NAS (ex. `/media/nas1/Documents` des deux côtés). C'est ce qui permet aux marqueurs déposés par un watcher d'être compris par l'autre : un marqueur désigne un chemin, et ce chemin doit signifier la même chose partout.

> 🆕 **Montages hétérogènes (Synology/QNAP, chemins différents).** Si le NAS voit les mêmes dossiers sous d'autres chemins que la machine locale — typiquement un Synology qui expose ses volumes en `/volume1/…` alors que la machine locale les monte en `/media/nas1/…` — vous n'êtes **pas** obligé de modifier vos montages. Déclarez la **correspondance des chemins de données** dans ⚙ Configuration → NAS (une ligne par volume : « Vu sur cette machine » ↔ « Vu sur le NAS »). Vos mappings restent construits avec les chemins de la machine locale ; le watcher NAS traduit automatiquement pour surveiller les bons dossiers et écrire des marqueurs compréhensibles côté local. La table est poussée au NAS avec le reste de la configuration, **propre à chaque utilisateur** (chaque machine déclare son propre montage, sans interférence si plusieurs postes partagent le même NAS). Laisser la table **vide** = chemins identiques (le cas d'un NAS Linux monté de la même façon).
>
> ℹ️ *Note pour les testeurs Synology :* DSM 7+ fournit systemd (unités `.service` standard dans `/etc/systemd/system`), un Python 3 intégré, et pip installable via `python3 -m ensurepip` puis un venv (pour `pyinotify`). Point important : **inotify local sur les volumes du NAS capte bien les écritures faites par les clients SMB/NFS** (c'est le serveur du NAS qui écrit sur le disque local, et inotify voit cette opération locale) — c'est exactement ce dont le watcher a besoin, puisqu'il tourne **sur** le NAS. Réserves connues : SSH à activer manuellement dans DSM, et persistance de l'unité/venv à vérifier après les mises à jour DSM. Cette configuration n'a pas été testée directement par l'auteur — retours bienvenus.
>
> 🆕 **Tester une correspondance (bouton « Test »).** À côté de chaque ligne de correspondance, un point coloré et un bouton **« Test »** vérifient que la correspondance fonctionne *de bout en bout*, sans jamais toucher au moteur de synchro. Le test écrit un fichier témoin depuis la machine locale, et le watcher NAS vérifie (1) qu'il le voit physiquement au bon endroit — c'est la **correspondance** — et (2) que son *inotify* capte bien cette écriture distante — c'est le **fonctionnement** (la particularité SMB/NFS). Résultat : 🟢 **vert** (tout marche), 🟡 **jaune** (les chemins correspondent mais un problème bloque — permissions, ou inotify qui ne remonte pas les écritures distantes), 🔴 **rouge** (les chemins ne désignent pas le même dossier — correspondance erronée). Cliquez le point pour le message détaillé. Éditer une ligne remet son point au gris (à re-tester). Le test nécessite que le watcher NAS tourne ; il fonctionne aussi avec une table vide (il valide alors les permissions et inotify sur les montages).
>
> ⚠️ **Garde-fou à l'ajout d'un mapping NAS.** Si vous ajoutez un dossier NAS dont la source dépend d'une correspondance non validée (jamais testée, ou dont le dernier test est jaune/rouge), l'application vous **avertit** et demande confirmation — elle ne bloque pas, mais vous invite à vérifier la correspondance d'abord. Cela évite d'ajouter en silence un mapping dont le montage est mal configuré. *(Rappel : même en cas de mauvaise correspondance, le moteur refuse toute suppression tant que la source n'est pas un montage réseau vivant — voir le garde-fou de suppression plus bas.)*
