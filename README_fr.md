...
- **Version de CLI testée : 0.8.0.** Le comportement du CLI change d'une version à l'autre, et seule celle-ci a été validée ici — les capacités décrites plus bas (type de média correct même avec une extension en majuscules, mise à la corbeille dans un dossier « Partagé avec moi ») sont apparues en 0.5.0 et restent acquises. L'application vérifie la version installée au démarrage et avertit si elle diffère (antérieure *ou* postérieure), en proposant de quitter pour installer la version testée. Le moteur, qui tourne aussi sans surveillance depuis les timers, se contente d'une ligne d'avertissement et n'interrompt jamais un passage.
...
En temps réel (`--subpath`), le moteur accepte la cible dès que **la cible elle-même OU son parent** est complet :

- Si le **parent** est complet, un **nouveau** dossier créé dedans est traité immédiatement (le cache de référence existe).
- Si la **cible** est complète, elle et tout son sous-arbre ont déjà été parcourus sans échec — une preuve encore plus forte, et rien d'inconnu à bâtir.
- Si ni l'un ni l'autre ne l'est (racine « tiède » héritée, enfants jamais parcourus), le temps réel **diffère au passage planifié** (code 3, message « dossier pas encore analysé — différé »). Il ne lance jamais un long parcours de découverte.

Tester la cible en plus du parent compte, parce que la complétude remonte de bas en haut sur *tous* les enfants non exclus : il suffit d'un enfant en échec pour que le parent devienne incomplet. Ne tester que le parent gelait donc tous les voisins sains d'un dossier cassé — un mapping entier pouvait cesser de se synchroniser en temps réel à cause d'un seul dossier. Le gel se limite désormais au sous-arbre réellement fautif.
...
### Dossiers impossibles à lire

Un dossier que le moteur ne peut pas lister — permissions changées à la source, ACL, montage — ne peut jamais être analysé. Son sous-arbre reste incomplet, la complétude n'atteint jamais la racine du mapping, et le temps réel s'y trouve désactivé pour tout ce qui est au-dessus. Le moteur le détectait et l'écrivait au journal depuis toujours, mais rien ne portait l'information une fois le passage terminé : la fenêtre se ferme, le journal défile, et un sous-arbre cesse d'être sauvegardé sans que personne le sache.

Le moteur **publie donc ce qu'il a constaté**. À la fin de chaque passage *complet*, il écrit `health.json` dans `~/.proton-drive-sync/` : pour chaque mapping, sa complétude et la liste des dossiers illisibles. Le fichier est réécrit à chaque passage complet, **y compris vide** — un problème résolu s'efface donc de lui-même ; un passage restreint (`--only-source`, `--reset-source`) fusionne au lieu de remplacer, puisqu'il n'a aucune vue sur les mappings qu'il n'a pas parcourus. Un passage temps réel ne l'écrit jamais.

Trois consommateurs lisent cette source unique :

- L'**icône de la barre des tâches** passe au motif d'avertissement et son infobulle nomme les dossiers.
- L'**interface** affiche une boîte en fin d'amorçage, qui liste les dossiers et dit clairement que leur contenu n'est pas sauvegardé, alors que tout le reste s'est synchronisé normalement.
- Le **consommateur temps réel** cesse de promettre une reprise impossible : quand un dossier est froid *parce qu'un dossier voisin est illisible*, il le dit et nomme le dossier à corriger, au lieu de « sera repris dès que son dossier parent sera indexé ».

Seuls les dossiers illisibles sont signalés ainsi. Les autres échecs du moteur — listing distant en échec, envoi refusé, orphelin non supprimé — sont transitoires et déjà retentés au passage suivant ; les signaler userait l'avertissement pour rien.
...
**Icône d'état dans la barre des tâches** (`tray_indicator.py`). Une double flèche circulaire près de l'horloge : **violette** = démons actifs + session Proton valide ; **violette avec un badge « ! » ambré** = démons actifs mais des scripts NAS attendent d'être poussés (voir « Alerte d'écart de scripts NAS » plus bas) ; **grise avec X rouge** = démons actifs mais, soit la session est expirée/le trousseau verrouillé, soit des dossiers ne sont pas sauvegardés (voir « Dossiers impossibles à lire ») ; **grise** = démons arrêtés. (Priorité : arrêtés > session expirée > dossiers non sauvegardés > scripts en attente > actif.) Les deux derniers partagent volontairement le motif au X rouge — un cinquième dessin aurait dilué le signal sans rien apprendre de plus ; l'**infobulle énumère toutes les conditions vraies en ce moment**, pas seulement celle qui a gagné la chaîne de priorité, et nomme les dossiers concernés. Seules les conditions dont on sait dire la cause sont signalées : un dossier illisible, ou une racine de mapping qu'aucun passage complet n'a jamais analysée (signalée au bout de deux heures, pour qu'un mapping fraîchement ajouté en attente de son passage planifié ne déclenche rien). Clic gauche = ouvrir l'éditeur ; clic droit = menu. Activable par la case « Barre des tâches » de ⚙ Configuration… (démarrage immédiat + autostart de session via `~/.config/autostart/` ; à la désactivation, l'applet s'éteint d'elle-même). Techniquement : XApp.StatusIcon (libxapp — natif Cinnamon/MATE/Xfce ; paquets `python3-gi` + `gir1.2-xapp-1.0`, préinstallés sur Mint). L'applet ne contacte **jamais** Proton : elle lit uniquement le battement de cœur `status.json` que le consommateur réécrit à chaque cycle dans `~/.proton-drive-sync/` — zéro sonde supplémentaire, zéro contention de trousseau. Nuance assumée : la sonde d'auth du consommateur n'a lieu que lorsqu'il y a du travail ; au repos, l'icône reflète le dernier état connu.
