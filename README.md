...
- **Tested CLI version: 0.8.0.** The CLI's behaviour changes between releases, and only this one has been validated here — the capabilities described further down (correct media type even with an uppercase extension, trashing inside a "Shared with me" folder) appeared in 0.5.0 and still hold. The application checks the installed version at startup and warns if it differs (older *or* newer), offering to quit so you can install the tested one. The engine, which also runs unattended from timers, only logs a warning and never stops a pass.
...
> **⚠ Deletion on a shared folder is potentially destructive.** A mapping writes into `<destination>/<name of your source folder>`, so that **subfolder** — not the whole shared folder — is what deletion acts on. With deletion enabled, everything inside it (and its own subfolders) that is missing locally is deleted and sent to the **owner's trash**, including files other people put there. Neither the rest of the shared folder nor the rest of the Drive is affected. Only enable it on a folder you are the sole contributor to, as a one-way delivery to its owner. Any other use risks erasing someone else's work. The application asks for an explicit confirmation when you enable deletion on such a destination, and again if you ask a reset to empty its remote folder.
...
In real time (`--subpath`), the engine accepts the target when **either the target itself or its parent** is complete:

- If the **parent** is complete, a **new** folder created inside it is handled immediately (the reference cache exists).
- If the **target** is complete, it and its whole subtree have already been traversed without failure — stronger evidence still, and nothing unknown to build.
- If neither is complete (an inherited "warm" root whose children were never traversed), real-time **defers to the scheduled pass** (exit code 3, "folder not analyzed yet — deferred"). It never launches a long discovery traversal.

Testing the target as well as the parent matters because completeness propagates bottom-up over *all* non-excluded children: a single failing child makes the parent incomplete. Testing the parent alone therefore froze every healthy sibling of a broken folder — a whole mapping could stop syncing in real time because of one folder. The freeze is now confined to the subtree that actually failed.
...
A folder the engine cannot list — permissions changed at the source, an ACL, a mount — can never be analyzed. Its subtree stays incomplete, completeness never reaches the mapping root, and real-time is disabled for everything above it. The engine detected this and wrote it to the log from the start, but nothing carried the information once the pass ended: the window closes, the log scrolls, and a subtree silently stops being backed up.

The engine therefore **publishes what it found**. At the end of every *full* pass it writes `health.json` in `~/.proton-drive-sync/`: per mapping, its completeness and the list of unreadable folders. The file is rewritten on every full pass **including when it is empty**, so a problem that has been fixed clears itself; a restricted pass (`--only-source`, `--reset-source`) merges instead of replacing, since it has no view of the mappings it did not traverse. A real-time pass never writes it.

Three consumers read that one source:

- The **tray icon** switches to the warning motif and its tooltip names the folders.
- The **GUI** shows a dialog at the end of priming, listing the folders and stating plainly that their contents are not backed up while everything else synced normally.
- The **real-time consumer** stops promising a resume that cannot happen: when a folder is cold *because a neighbouring folder cannot be read*, it says so and names the folder to fix, instead of "will be picked up as soon as its parent folder is indexed".

Only unreadable folders are reported this way. The engine's other failures — a remote listing that fails, an upload refused, an orphan that could not be deleted — are transient and already retried on the next pass; reporting them would spend the warning for nothing.
