...
- **Tested CLI version: 0.6.0.** The CLI's behaviour changes between releases, and only this one has been validated here — the capabilities described further down (correct media type even with an uppercase extension, trashing inside a "Shared with me" folder) appeared in 0.5.0 and still hold. The application checks the installed version at startup and warns if it differs (older *or* newer), offering to quit so you can install the tested one. The engine, which also runs unattended from timers, only logs a warning and never stops a pass.
